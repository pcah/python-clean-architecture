...
* **Which version of `python-clean-architecture` are you using?** You can get the exact version by running `python -c "import pca; print(pca.VERSION.as_string())"` in your terminal.
...
You will need Poetry to start contributing on the `python-clean-architecture` codebase. Refer to the [documentation](https://python-poetry.org/docs/) to start using Poetry.
...
Now, you will need to install the required dependency for `python-clean-architecture` and be sure that the current tests are passing on your machine:
...
`python-clean-architecture` uses the [black](https://github.com/psf/black) coding style and you must ensure that your code follows it. If not, the CI will fail and your Pull Request will not be merged.
