...
from pca.data.traits import undefined_value
...
def test_construct_validators(mocker):
    ...
    genus = object()
    with mocker.patch('pca.data.traits.trait.construct_validators'):
        nature_class_factory(
            trait_name='a_trait', nature_name='ANature',
            kwargs={'genus': genus}
        )
        from pca.data.traits.trait import construct_validators
